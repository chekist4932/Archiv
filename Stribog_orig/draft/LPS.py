...
L_matrix = ['8e20faa72ba0b470', '47107ddd9b505a38', 'ad08b0e0c3282d1c', 'd8045870ef14980e', '6c022c38f90a4c07',
...
def hex_to_bin_v3(mess_bytes: bytes) -> int or str:  # mess.encode("utf-8")
...
def hex_to_bin(hex_str: str):
    ...
    for i in range(0, len(hex_str), 2):
        char = hex_str[i:i + 2]
        binary_str += bin(int(char, 16))[2:].zfill(8)
    return binary_str
...
def bin_to_hex(res):
    ...
    for i in [res[x:x + 8] for x in range(0, len(res), 8)]:
        hex_str += hex(int(i, 2))[2:].zfill(2)
    return hex_str


def XOR_(vec_1: str, vec_2: str, dimension):
    result = ''
    for i in range(dimension):
        result += str((int(vec_1[i]) + int(vec_2[i])) % 2)
    return result
...
def l_trans(p_res, l):
    ...
    # print([bin_to_hex(x) for x in res_list[0]])
    # input()
...
def E_trans(k1, m):  # hex, bin
    ...
    k[0] = k1
    for i in range(1, 13):
        k[i] = LPS(hex_to_bin(X_trans(hex_to_bin(k[i - 1]), hex_to_bin(C_const[i - 1]))), S_const, P_const, L_matrix)
    ...
    for j in range(13):
        if j == 12:
            ...
            break
        m = hex_to_bin(X_trans(hex_to_bin(k[j]), m))
        m = hex_to_bin(LPS(m, S_const, P_const, L_matrix))

    return bin_to_hex(m)


def g_N(h, m, N):  # bin,bin,bin
    k1 = LPS(XOR_(h, N, 512), S_const, P_const, L_matrix)
    # print(f"k1 - {k1}")
    e_res = E_trans(k1, m)
    # print(f"msg - {bin_to_hex(m)}")
    # print(f"e_res - {e_res}")
    # print(f"hash_ - {bin_to_hex(h)}")
    # print(f"e_ and h_ - {e_res}\n{h}")
    mp = XOR_(hex_to_bin(e_res), h, 512)
    # print(f"mp - {bin_to_hex(mp)}")
    h = XOR_(mp, m, 512)
    return h  # bin
